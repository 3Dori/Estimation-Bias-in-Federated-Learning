...
class FederatedLearningTrainer:
    ...
    def __init__(self, train_loaders, test_loader,
                 ...
                 model=None, model_parameters={}, model_name='MLP', criterion=None, is_classification=True,
                 use_cuda=True,
        ...
        model = model or FederatedLearningTrainer.MODELS[model_name]
        self.models = [model(**model_parameters).to(self.device) for k in range(K)]
        ...
        self.global_model = model(**model_parameters).to(self.device)
    ...
    def train(self, test_loss_out=None, accuracy_out=None):
        for global_round in range(self.n_global_rounds):
            ...
            if test_loss_out is not None:
                test_loss_out.append(test_loss)
            if accuracy_out is not None:
